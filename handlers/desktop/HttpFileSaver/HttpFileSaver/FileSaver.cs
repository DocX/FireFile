...
namespace HttpFileSaver
{
	...
	{		
		...
		public class RootMapping
		{
			...
			/// <summary>
			/// The local root path, where are files saved. Must end with /
			/// </summary>
			public string localRootPath {
				get { return _localRootPath; }
				set {
					if (!value.EndsWith("/"))
						throw new ArgumentException("File root path must end with slash /");
					_localRootPath = value;
				}
			}
		}
